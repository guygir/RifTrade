...
async function addCardToCanvas(
  ...
  if (card.image_url) {
    try {
      ...
      // Convert PDF mm dimensions to pixels at 186 DPI for badges (middle between 300 and 72)
      // 1mm = 186/25.4 ≈ 7.32px
      const mmToPx = 186 / 25.4;
      
      // Convert for text at 93 DPI (50% of badge DPI to maintain same ratio as before)
      // Original ratio: badges 300 DPI, text 150 DPI (50%)
      // Now: badges 186 DPI, text 93 DPI (50%)
      // 1mm = 93/25.4 ≈ 3.66px
      const mmToPxText = 93 / 25.4;
      
      // Add quantity badge if quantity > 1 (matches PDF style exactly)
      // PDF: circle at x + width - 5mm, y + 5mm, radius 3mm
      if (card.quantity && card.quantity > 1) {
        const badgeRadius = 3 * mmToPx; // 3mm radius at 186 DPI
        ...
        ctx.font = 'bold ' + (8 * mmToPxText) + 'px Arial'; // 8pt font scaled at 93 DPI (50% of badge size)
      ...
      if (isFoil) {
        ...
        const badgeWidth = 12 * mmToPx; // 12mm wide at 186 DPI
        const badgeHeight = 5 * mmToPx; // 5mm tall at 186 DPI
        
        ctx.fillStyle = '#2563eb'; // Blue (same as PDF: RGB(37, 99, 235))
        ctx.fillRect(badgeX, badgeY, badgeWidth, badgeHeight);
        
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold ' + (7 * mmToPxText) + 'px Arial'; // 7pt font scaled at 93 DPI (50% of badge size)
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
...
export async function generateCardListPNG(
  ...
  // Convert PDF dimensions (mm) to pixels at 150 DPI for optimized file size
  // Reduced from 300 DPI to significantly reduce PNG file size while maintaining good quality
  // PDF uses A4: 210mm x 297mm
  // At 150 DPI: 1mm = 5.91px (half the resolution = ~4x smaller file size)
  const dpi = 150;
  ...
  // Account for header and title (header ~24px + title ~48px + spacing ~60px = ~132px)
  const headerHeightPx = 132; // Header + title + spacing
  ...
  for (let pageNum = 0; pageNum < totalPages; pageNum++) {
    ...
    // Header text (centered, 75% of previous 32px = 24px)
    ctx.fillStyle = '#646464';
    ctx.font = '24px Arial';
    ctx.textAlign = 'center';
    ...
    // Title (centered, 75% of previous 64px = 48px)
    let yPos = marginPx + 40;
